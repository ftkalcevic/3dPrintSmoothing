...
#    Modified by Frank Tkalcevic (frank@franksworkshop.com.au) to be used as
#    part
...
def dist_lseg(l1, l2, p):
    ...
    x0, y0, z0, a0, f0, m0 = l1
    xa, ya, za, aa, fa, ma = l2
    xi, yi, zi, ai, fi, mi = p
...
def arc_center(plane, p1, p2, p3):
    x1, y1, z1, a1, f1, m1 = p1
    x2, y2, z2, a2, f2, m2 = p2
    x3, y3, z3, a3, f3, m3 = p3
...
def one_quadrant(plane, c, p1, p2, p3):
    ...
    signs = set(((sign(x1 - xc),sign(y1 - yc)),
...
def test_arcs():
    def test(s,m,e,ccw):
        ...
        #log( "test " + str(s) + ", " + str(m) + ", " + str(e) + ", " + ("CCW"
        #if ccw else "CW") + " = " + ("CCW" if is_ccw else "CW") )
        if is_ccw != ccw:
...
def douglas(st, tolerance=.001, length_tolerance=0.005, plane=None, _first=True, extrudeRelative = False):
    """\
...
"""
    ...
    for i_st, point_st in enumerate(st):
        ...
        extrusion_length = 0       # remember extrusion for relative extrusions
        ...
        if worst_arc_dist < tolerance and worst_arc_dist < worst_dist and abs(chord_length(plane,(c1,c2),ps,path[max_arc], pe) - path_length) < (len(path) - 1) * length_tolerance :
            ...
            if ccw:
                last_path.append(["G3", [path[-1][0],path[-1][1],path[-1][2],extrusion_length,path[-1][4],path[-1][5]], arc_fmt(plane, c1, c2, ps)])
            else:
                last_path.append(["G2", [path[-1][0],path[-1][1],path[-1][2],extrusion_length,path[-1][4],path[-1][5]], arc_fmt(plane, c1, c2, ps)])
